...
const index = async (req, res) => {
  ...
  try {
    const ratings = await db.sequelize.query(
      "SELECT Reviews.id,Reviews.comment,Reviews.rating,Reviews.status,Users.firstName,Users.lastName FROM Reviews JOIN Users ON Users.id = Reviews.authorID OR Users.id = Reviews.agentID", { type: sequelize.QueryTypes.SELECT }
    );
    if (!ratings) {

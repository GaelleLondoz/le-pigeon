...
const index = async(req, res) => {
    ...
    try {
        const bookings = await db.sequelize.query(
            "SELECT Bookings.id,Bookings.date,Bookings.status,Users.firstName,Users.lastName FROM Bookings JOIN Users ON Users.id = Bookings.userID OR Users.id = Bookings.agentID", { type: sequelize.QueryTypes.SELECT }
        );
        if (!bookings) {
...
const create = async(req, res) => {
    ...
    if (newBooking.date === "" || newBooking.date === undefined) {
        ...
        errors.push({
            target: "date",
            msg: "La date de la réservation doit être supérieure à la date d'aujourd'hui !",
        });
    }
    ...
    if (newBooking.hours === null || newBooking.hours === "") {
        errors.push({
            target: "hours",
            msg: "Veuillez séléctionner le nombre d'heure que vous souhaitez réserver !",
        });
    }
...
const updateDateBooking = async(req, res) => {
    ...
    if (!compareDateForUpdateBooking(date)) {
        errors.push({
            target: "date",
            msg: "La date de réservation doit être supérieure à la date d'aujourd'hui plus un jour !",
        });
        return res.status(400).json({ errors });
    }
